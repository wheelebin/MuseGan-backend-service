...
from .packages import pypianoroll
...
def pianoroll_to_image(
    ...
    if colormap is not None:
        pianoroll = np.matmul(1.0 - colormap, np.expand_dims(pianoroll, -1))
        pianoroll = pianoroll.squeeze(-1).clip(0.0, 1.0)
    ...
    if boundary_width:
        quantized = np.pad(
            ...
            "constant",
            constant_values=boundary_color,
        )
    ...
    if colormap is None:
        ...
        image = np.reshape(
            ...
            (
                ...
                transposed.shape[3] * transposed.shape[4],
                1,
            ),
        )
    else:
...
def image_pair(
    image1, image2, mode="side-by-side", boundary_width=1, boundary_color=0, frame=False
):
    """
    ...
    if image1.ndim != image2.ndim:
        raise ValueError(
            "Input image arrays must have the same number of " "dimensions."
        )
    if not (mode == "side-by-side" or mode == "top-down"):
        raise ValueError(
            "Invalid mode received. Valid modes are " "'side-by-side' and 'top-down'."
        )
...
def image_grid(image, grid_shape, grid_width=3, grid_color=0, frame=True):
    ...
    if grid_width:
        sliced = np.pad(
            sliced,
            ((0, 0), (grid_width, 0), (grid_width, 0), (0, 0)),
            "constant",
            constant_values=grid_color,
        )

    # Reshape to split the first (batch) axis into two axes
    reshaped = np.reshape(sliced, ((grid_shape[0], grid_shape[1]) + sliced.shape[1:]))
    ...
    grid = np.reshape(
        transposed,
        (
            grid_shape[0] * transposed.shape[1],
            grid_shape[1] * transposed.shape[3],
            image.shape[-1],
        ),
    )
...
def save_pianoroll(
    ...
    if pianoroll.shape[-1] != len(programs):
        raise ValueError(
            "Length of `programs` does not match the number of "
            "tracks for the input array."
        )
    if pianoroll.shape[-1] != len(is_drums):
        raise ValueError(
            "Length of `is_drums` does not match the number of "
            "tracks for the input array."
        )

    reshaped = pianoroll.reshape(
        -1,
        pianoroll.shape[1] * pianoroll.shape[2],
        pianoroll.shape[3],
        pianoroll.shape[4],
    )
    ...
    padded = np.pad(
        reshaped,
        ((0, 0), (0, pianoroll.shape[2]), (lowest_pitch, to_pad_pitch_high), (0, 0)),
        "constant",
    )
