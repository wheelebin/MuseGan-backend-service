...
class Track:
    ...
    def _validate_type(self, attr):
        ...
        if attr == "program":
            if not isinstance(self.program, int):
                raise TypeError(
                    "`program` must be of type int, not " f"{type(self.program)}."
                )
        elif attr == "is_drum":
            if not isinstance(self.is_drum, bool):
                raise TypeError(
                    "`is_drum` must be of type bool, not " f"{type(self.is_drum)}."
                )
        elif attr == "name":
            if not isinstance(self.name, str):
                raise TypeError(f"`name` must be of type str, not {type(self.name)}.")
        elif attr == "pianoroll":
            if not isinstance(self.pianoroll, ndarray):
                raise TypeError(
                    "`pianoroll` must be a NumPy array, not " f"{type(self.pianoroll)}."
                )
    ...
    def trim(
        ...
        if end is None:
            ...
            raise ValueError("`end` must be shorter than the piano roll length.")
...
class StandardTrack(Track):
    ...
    def _validate(self, attr):
        ...
        if attr == "pianoroll" and np.any(self.pianoroll > 127):
            raise ValueError("`pianoroll` must contain only integers between 0 to 127.")
    ...
    def clip(self: _StandardTrack, lower: int = 0, upper: int = 127) -> _StandardTrack:
...
class BinaryTrack(Track):
    ...
    def _validate_type(self, attr):
        ...
        if attr == "pianoroll" and self.pianoroll.dtype != np.bool_:
            raise TypeError(
                "`pianoroll` must be of data type bool, not " f"{self.pianoroll.dtype}."
            )
