...
def load_data(data_source, data_filename):
    ...
    raise ValueError(
        "Expect `data_source` to be one of 'sa', 'npy', 'npz'. "
...
def get_dataset(
    ...
    if labels is None:
        dataset = tf.data.Dataset.from_generator(lambda: _gen_data(data), tf.float32)
        if use_random_transpose:
            dataset = dataset.map(
                lambda pianoroll: tf.py_func(random_transpose, [pianoroll], tf.float32),
                num_parallel_calls=num_threads,
            )
        dataset = dataset.map(
            lambda pianoroll: set_pianoroll_shape(pianoroll, data_shape),
            num_parallel_calls=num_threads,
        )
    else:
        assert len(data) == len(labels), "Lengths of `data` and `lables` do not match."
        ...
        dataset = dataset.map(
            lambda pianoroll, label: (
                set_pianoroll_shape(pianoroll, data_shape),
                set_label_shape(label),
            ),
            num_parallel_calls=num_threads,
        )
