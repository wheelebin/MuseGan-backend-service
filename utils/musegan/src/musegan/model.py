...
class Model:
    ...
    def __init__(self, params, name="Model"):
        ...
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE) as scope:
            ...
            self.dis = load_component(
                "discriminator", params["nets"]["discriminator"], "Discriminator"
            )(
                n_tracks=params["data_shape"][-1],
    ...
    def __call__(
        self, x=None, z=None, y=None, c=None, mode=None, params=None, config=None
    ):
        if mode == "train":
            if x is None:
        ...
        raise ValueError(
            "Unrecognized mode received. Expect 'train' or "
    ...
    def get_train_nodes(self, x, z=None, y=None, c=None, params=None, config=None):
        ...
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE) as scope:
            ...
            nodes["gen_step"] = tf.get_variable(
                "gen_step", [], tf.int32, tf.constant_initializer(0), trainable=False
            )
            ...
            nodes["slope"] = tf.get_variable(
                "slope", [], tf.float32, tf.constant_initializer(1.0), trainable=False
            )
            ...
            if config["use_slope_annealing"]:
                ...
                scheduled_slope = get_scheduled_variable(
                    1.0,
                    slope_schedule["end_value"],
                    slope_schedule["start"],
                    slope_schedule["end"],
                )
                tf.add_to_collection(
                    tf.GraphKeys.UPDATE_OPS, tf.assign(nodes["slope"], scheduled_slope)
                )
            ...
            gen_opt = tf.train.AdamOptimizer(
                nodes["learning_rate"], config["adam"]["beta1"], config["adam"]["beta2"]
            )
            ...
            if config["save_summaries_steps"] > 0:
                ...
                if config["use_learning_rate_decay"]:
                    with tf.name_scope("learning_rate_decay"):
                        tf.summary.scalar("learning_rate", nodes["learning_rate"])
                if config["use_slope_annealing"]:
    ...
    def get_predict_nodes(self, z=None, y=None, c=None, params=None, config=None):
        ...
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            def _get_filepath(folder_name, name, suffix, ext):
                ...
                if suffix:
                    if isinstance(suffix, str):
                        ...
                        suffix_ = str(suffix, "utf8")
                    return os.path.join(
                        config["result_dir"],
                        folder_name,
                        name,
                        "{}_{}.{}".format(name, suffix_, ext),
                    )
                return os.path.join(
                    config["result_dir"], folder_name, name, "{}.{}".format(name, ext)
                )
            ...
            # --- Save array ops -----------------------------------------------
            if "collect_save_arrays_op" in config:
                ...
                for key, value in arrays.items():
                    save_array_ops.append(
                        tf.py_func(
                            lambda array, suffix, k=key: _save_array(array, suffix, k),
                            [value, config["suffix"]],
                            tf.int32,
                        )
                    )
            ...
            # --- Save image ops -----------------------------------------------
            if "collect_save_images_op" in config:
                ...
                images = {"fake_x": 0.5 * (nodes["fake_x"] + 1.0)}
                if params["use_binary_neurons"]:
                    images["fake_x_preactivated"] = 0.5 * (
                        nodes["fake_x_preactivated"] + 1.0
                    )
                else:
                ...
                for key, value in images.items():
                    save_image_ops.append(
                        tf.py_func(
                            lambda array, suffix, k=key: _save_images(array, suffix, k),
                            [value, config["suffix"]],
                            tf.int32,
                        )
                    )
                    ...
                    make_sure_path_exists(
                        os.path.join(config["result_dir"], "images", key + "_colored")
                    )
                nodes["save_images_op"] = tf.group(save_image_ops)

            # --- Save pianoroll ops -------------------------------------------
            if "collect_save_pianorolls_op" in config:

                def _save_pianoroll(array, suffix, name):
                    filepath = _get_filepath("pianorolls", name, "0", "npz")
                ...
                for key, value in pianorolls.items():
                    ...
                    funccy = tf.py_func(
                        lambda array, suffix, k=key: _save_pianoroll(array, suffix, k),
                        [value, config["suffix"]],
                        tf.float32,
                    )

                    save_pianoroll_ops.append(funccy)
