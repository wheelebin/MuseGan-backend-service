...
def setup():
    ...
    if params.get("is_accompaniment") and params.get("condition_track_idx") is None:
        raise TypeError(
            "`condition_track_idx` cannot be None type in " "accompaniment mode."
        )
...
def main():
    ...
    model = Model(params)
    if params.get("is_accompaniment"):
        train_c = tf.expand_dims(train_x[..., params["condition_track_idx"]], -1)
        train_nodes = model(
    ...
    def get_n_params(var_list):
        ...
        return int(
            np.sum([np.product([x.value for x in var.get_shape()]) for var in var_list])
        )

    LOGGER.info(
        "Number of trainable parameters in {}: {:,}".format(
    ...
    for component in model.components:
        LOGGER.info(
            "Number of trainable parameters in {}: {:,}".format(
                component.name,
                get_n_params(tf.trainable_variables(model.name + "/" + component.name)),
            )
        )
    ...
    if config["save_samples_steps"] > 0:
        ...
        sampler_op_no_pianoroll = tf.group(
            [
                predict_nodes[key]
                for key in ("save_arrays_op", "save_images_op")
                if key in predict_nodes
            ]
        )
    ...
    if config["evaluate_steps"] > 0:
        binarized = tf.round(0.5 * (predict_nodes["fake_x"] + 1.0))
    ...
    with tf.train.MonitoredTrainingSession(
        ...
        while step < config["steps"]:
            ...
            # Run sampler
            if (config["save_samples_steps"] > 0) and (
                step % config["save_samples_steps"] == 0
            ):
                ...
                if step < 3000:
                    sess.run(sampler_op_no_pianoroll, feed_dict=feed_dict_sampler)
                else:
                    sess.run(sampler_op, feed_dict=feed_dict_sampler)

            # Run evaluation
            if (config["evaluate_steps"] > 0) and (
                step % config["evaluate_steps"] == 0
            ):
                ...
                feed_dict_evaluation = {
                    placeholder_z: scipy.stats.truncnorm.rvs(
                        -2,
                        2,
                        size=(np.prod(config["sample_grid"]), params["latent_dim"]),
                    )
                }
                if params.get("is_accompaniment"):
